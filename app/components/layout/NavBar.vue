...
import {TerminalSquare, Hamburger} from "lucide-vue-next";
...
<template>
  <nav
    ...
    <div class="lg:hidden mr-2 cursor-pointer flex items-center" tabindex="0">
      <Hamburger/>
    </div>
    ...
    <ul class="gap-4 hidden lg:flex">
      <li
          ...
          @click="clickLink(link.link)"
      >
        ./<span :class="{'text-primary': link.active}" class="hover:text-primary lowercase">{{ link.label }}</span>
      </li>
      <li class="h-full flex items-center justify-center mr-1" tabindex="0" @click="clickLink('https://github.com/bendavies99', '_tab')">
